...
select_next_lines() {
  ...
  [[ -n "$max_count" ]] || { echo "[ERROR] max_count required" >&2; exit 2; }

  local params_clean tracker_clean tmp_params tmp_tracker
  tmp_params="$(mktemp)"; tmp_tracker="$(mktemp)"
...
append_tracker() {
  local tracker_file="$1"; shift
  [[ -n "$tracker_file" ]] || { echo "[ERROR] tracker_file required" >&2; exit 2; }
  [[ $# -gt 0 ]] || { echo "[ERROR] at least one line to append required" >&2; exit 2; }
...
case "$1" in
  select_next_lines) shift; select_next_lines "$@";;
  append_tracker) shift; append_tracker "$@";;
