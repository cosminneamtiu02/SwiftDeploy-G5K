...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --machine-path) MACHINE_PATH="$2"; shift 2;;
    --fe-path) FE_PATH="$2"; shift 2;;
...
[[ -n "$MACHINE_PATH" ]] || { echo "[ERROR] --machine-path required" >&2; exit 2; }
[[ -n "$FE_PATH" ]] || { echo "[ERROR] --fe-path required" >&2; exit 2; }
