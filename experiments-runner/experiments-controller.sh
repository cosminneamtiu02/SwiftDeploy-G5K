...
log_info()  { echo -e "$(log_ts) ${GREEN}[INFO]${RESET} $*"; }
log_warn()  { echo -e "$(log_ts) ${YELLOW}[WARN]${RESET} $*" >&2; }
log_err()   { echo -e "$(log_ts) ${RED}[ERROR]${RESET} $*" >&2; }
log_debug() { if [[ "$VERBOSE" == true ]]; then echo -e "$(log_ts) ${DIM}[DEBUG]${RESET} $*"; fi }
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    -c|--config) CONFIG_FILE_NAME="$2"; shift 2;;
    -p|--phases) REQUESTED_PHASES="$2"; shift 2;;
    --dry-run) DRY_RUN=true; shift;;
    --verbose) VERBOSE=true; shift;;
    --continue-on-error) CONTINUE_ON_ERROR=true; shift;;
    --manual) FORCE_MANUAL_INSTANTIATION=true; shift;;
    --log-dir) LOG_DIR="$2"; shift 2;;
    --no-color) NO_COLOR=true; shift;;
    --no-live-logs) LIVE_LOGS=false; shift;;
...
require_cmd() { command -v "$1" >/dev/null 2>&1 || { log_err "Required command not found: $1"; exit 2; }; }
...
[[ -f "${CONFIG_JSON}" ]] || { log_err "Config file not found in ${CONFIG_DIR_ABS}: ${CONFIG_FILE_NAME}"; exit 2; }
...
ENV_LIST_JSON="$(json_or_empty '.machine_setup.env_variables_list')"
...
phase_instantiate() {
  ...
  run_step "Phase 1/4: Machine instantiation" "$(phase_log 01-instantiate.log)" bash "${script}"
}
...
phase_prepare() {
  ...
  run_step "Phase 2/4: Project preparation" "$(phase_log 02-prepare.log)" bash "${PREPARE_SCRIPT}" \
...
phase_delegate() {
  ...
  run_step "Phase 3/4: Experiment delegation" "$(phase_log 03-delegate.log)" bash "${DELEGATOR_SCRIPT}" \
...
phase_collect() {
  ...
  run_step "Phase 4/4: Results collection" "$(phase_log 04-collect.log)" bash "${COLLECT_ONM_SCRIPT}" \
...
for p in "${PHASES[@]}"; do
  case "$p" in
    instantiate) if ! phase_instantiate; then phase_status=$?; log_err "Phase 'instantiate' failed (${phase_status})"; $CONTINUE_ON_ERROR || exit "$phase_status"; fi;;
    prepare)     if ! phase_prepare;     then phase_status=$?; log_err "Phase 'prepare' failed (${phase_status})";     $CONTINUE_ON_ERROR || exit "$phase_status"; fi;;
    delegate)    if ! phase_delegate;    then phase_status=$?; log_err "Phase 'delegate' failed (${phase_status})";    $CONTINUE_ON_ERROR || exit "$phase_status"; fi;;
    collect)     if ! phase_collect;     then phase_status=$?; log_err "Phase 'collect' failed (${phase_status})";     $CONTINUE_ON_ERROR || exit "$phase_status"; fi;;
    "" ) ;;
